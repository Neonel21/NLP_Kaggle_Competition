...
##figure out the distribution shape
##  df= (7613, 5) df_test= (3263, 4)
...
#to see if we can find any special patterns
...
#histogram of the number of words in the headlines of each topic:
f, (ax1, ax2,) = plt.subplots(1,2,figsize=(25,8))
...
#to have more examples where the model can learn
...
df_test['text']=df['text'].apply(lambda x : remove_URL(x))
df_test['text']=df['text'].apply(lambda x : remove_html(x))
df_test['text']=df['text'].apply(lambda x : remove_emoji(x))


df_test['text']=df_test['text'].str.replace('\d+', '')
df['text']=df_test['text'].str.replace('\d+', '')

df_test['text'] = df_test['text'].apply(lambda s : cleaner(s))
df['text'] = df_test['text'].apply(lambda s : cleaner(s))

df_test['text']=df_test['text'].apply(lambda x : remove_punct(x))
df['text']=df_test['text'].apply(lambda x : remove_punct(x))
...
df['text']=df_test['text'].str.replace('   ', ' ')
df['text']=df_test['text'].str.replace('     ', ' ')
df['text']=df_test['text'].str.replace('\xa0 \xa0 \xa0', ' ')
df['text']=df_test['text'].str.replace('  ', ' ')
df['text']=df_test['text'].str.replace('—', ' ')
df['text']=df_test['text'].str.replace('–', ' ')
...
percentages = model_selection.cross_val_score(clf, train_vectors, df["target"], cv=3, scoring="f1")
print(percentages)
